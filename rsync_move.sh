...
while true; do
    ...
    if [[ "$selection" =~ ^[0-9]+$ ]]; then
        ...
        if [ "$selection" -ge 1 ] && [ "$selection" -le ${#valid_folders[@]} ]; then
            ...
            nohup bash -c "rsync -ah --progress --remove-source-files \"$source_directory/$selected_item\" \"$destination_directory\" && echo -e '\e[1;36m项目成功移动到 $destination_folder_name!\e[0m'" > "out$counter" 2>&1 &
            ((counter++))
        else
