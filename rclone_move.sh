...
while true; do
    ...
    if [[ "$selection" =~ ^[0-9]+$ ]] && [ "$selection" -ge 1 ] && [ "$selection" -le ${#files[@]} ]; then
        ...
        nohup bash -c "echo -e '\e[1;36m项目 $selected_item\e[0m' && rclone move \"$source_path/$selected_item\" \"$destination_path/$selected_item\" --progress --stats-one-line --stats 5s && echo -e '\e[1;36m项目成功移动到 $destination_path/$selected_item!\e[0m'" > "out$counter" 2>&1 &
